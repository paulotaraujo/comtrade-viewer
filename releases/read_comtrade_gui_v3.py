...
class ComtradeViewer(QMainWindow):
    ...
    def export_selected(self):
        ...
        out_cfg_str, _ = QFileDialog.getSaveFileName(self, "Salvar novo .cfg", suggested, "CFG (*.cfg);;All (*.*)")
        if not out_cfg_str:
            return
        ...
        name_to_col = {a.name: i for i, a in enumerate(self.cfg.analogs)}
        cols: List[int] = [name_to_col[nm] for nm in names if nm in name_to_col]
        if not cols:
        ...
        QMessageBox.information(self, "Exportar seleção", f"Arquivos gerados com sucesso:\n\nCFG: {out_cfg}\nDAT: {out_dat}")

    # ---------- Export BINARY (.cfg+.bdat) ----------
    def export_selected_binary(self):
        if self.cfg is None or self.t is None or self.analog_raw is None:
            return
        ...
        if out_cfg.suffix.lower() != ".cfg":
            out_cfg = out_cfg.with_suffix(".cfg")
        out_bdat = out_cfg.with_suffix(".bdat")
        ...
        try:
            ...
            # tenta int32; se der ruim, cai para int64
            try:
                write_bdat_binary(out_bdat, t_sel, raw_sel, new_cfg, ts_mode="i")
            except Exception:
                write_bdat_binary(out_bdat, t_sel, raw_sel, new_cfg, ts_mode="q")
        except Exception as e:
            QMessageBox.critical(self, "Exportar seleção (BINARY)", f"Falha ao exportar:\n{e}")
            return

        QMessageBox.information(self, "Exportar seleção (BINARY)", f"Arquivos gerados com sucesso:\n\nCFG: {out_cfg}\nBDAT: {out_bdat}")
    ...
    def export_as_dialog(self):
        ...
        lay = QVBoxLayout(dlg)
        lay.addWidget(QLabel("Escolha o formato de exportação:"))

        lst = QListWidget()
        lst.setSelectionMode(QListWidget.SingleSelection)

        it1 = QListWidgetItem("COMTRADE ASCII (.cfg + .dat)")
        it1.setData(Qt.UserRole, "comtrade_ascii")
        lst.addItem(it1)

        it2 = QListWidgetItem("COMTRADE BINARY (.cfg + .bdat)")
        it2.setData(Qt.UserRole, "comtrade_bin")
        lst.addItem(it2)

        it3 = QListWidgetItem("JSON")
        it3.setData(Qt.UserRole, "json")
        lst.addItem(it3)
        ...
        choice = item.data(Qt.UserRole) if item else "comtrade_ascii"

        if choice == "comtrade_ascii":
            self.export_selected()
        elif choice == "comtrade_bin":
            self.export_selected_binary()
        elif choice == "json":
            self.export_selected_json()
        else:
    ...
    def export_selected_json(self):
        ...
        out_json_str, _ = QFileDialog.getSaveFileName(self, "Salvar JSON", suggested, "JSON (*.json);;All (*.*)")
        if not out_json_str:
            return
